...
def main(genomas, config):
    ...
    while rodando:
        ...
        indice_cano = 0
        if len(passaros) > 0:
            if len(canos) > 1 and passaros[0].x > (canos[0].CANO_TOPO.get_width()):
                indice_cano = 1
        else:
            rodando = False
            break
...
def rodar(caminho_config):
    ...
    população = neat.Population(config)
    população.add_reporter(neat.StdOutReporter(True))
    população.add_reporter(neat.StatisticsReporter())
    população.run(main, 25)


def key(genoma):
    return genoma.fitness
